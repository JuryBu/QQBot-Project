...
§五 D8 + S4_设计决策.md D7 轮龄为主+命中修正 / D8 summary 封板）：
  ...
  ⑦ 降档守护以「有无 summary_text」为准（S4 批7 解耦 sealed）：有 summary 即可降档、
     无 summary 才强制 full（D8 防读空摘要）
...
def test_build_contexts_unsealed_with_summary_downgrades(tmp_ckpt):
    """S4 批7 改契约：未封板【但有 summary_text】的组按 age 正常降档（不再强制 full）。
    旧契约「未封板即强制 full」把降档错误耦合到封板，真机致真轮组永 full（死代码）。
    新契约：防空洞的条件是 summary 文本存在性，非 sealed。age=90 → brief，brief_text
    空 → _select_tier_body 回退 summary_text（OPEN_SUM）。"""
    mgr = TFileManager()
    g = _sealed_group("rg000001", 1, 10, full="OPEN_FULL", summary="OPEN_SUM", brief="")
    g["sealed"] = False  # 未封板，但有 summary_text
    t_file = _make_t_file([g], next_round_id=101)  # age=90 → brief
    ctxs = mgr.build_llm_contexts(t_file, window_key="GroupMessage:g2", record_cfg=CFG)
    blob = "\n".join(c.get("content", "") for c in ctxs if isinstance(c, dict))
    assert "OPEN_SUM" in blob       # 降档读 summary（brief 缺文本回退 summary）
    assert "OPEN_FULL" not in blob  # 不再强制 full


def test_build_contexts_unsealed_no_summary_forced_full(tmp_ckpt):
    """对照：未封板【且无 summary_text】的组仍强制 full（防空洞）。"""
    mgr = TFileManager()
    g = _sealed_group("rg000001", 1, 10, full="OPEN_FULL", summary="", brief="")
    g["sealed"] = False  # 未封板且无 summary
    t_file = _make_t_file([g], next_round_id=101)  # age=90 本应 brief，但无 summary → full
    ctxs = mgr.build_llm_contexts(t_file, window_key="GroupMessage:g2", record_cfg=CFG)
    blob = "\n".join(c.get("content", "") for c in ctxs if isinstance(c, dict))
    assert "OPEN_FULL" in blob  # 无 summary → 强制 full 防空洞
...
# ============================================================
# ⑦ 降档守护以「有无 summary_text」为准（S4 批7：解耦 sealed，非「只 sealed 才降」）
# ============================================================
def test_unsealed_with_summary_can_downgrade():
    """S4 批7 改契约：未封板但有 summary_text 的组照样按 age 降档（旧契约误强制 full）。
    _sealed_group 默认带非空 summary → age=200 极老 → brief。"""
    g = dict(_sealed_group("rg000001", 1, 10), sealed=False)  # 有 summary_text
    assert record.tier_for_group(g, now_round=210, hit_table={}, cfg=CFG,
                                 now_ts=1000.0) == record.TIER_BRIEF


def test_no_summary_text_forced_full():
    """无 summary_text 的组（无论 sealed 与否）强制 full，防降档读空摘要造空洞。"""
    g = dict(_sealed_group("rg000001", 1, 10, summary=""), sealed=False)
    assert record.tier_for_group(g, now_round=210, hit_table={}, cfg=CFG,
                                 now_ts=1000.0) == record.TIER_FULL


def test_sealed_group_can_downgrade():
    """已封板组可正常降档（对照组，证明守护不误伤 sealed 组）。"""
