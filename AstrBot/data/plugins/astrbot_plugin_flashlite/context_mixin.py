...
{"发送者: " + sender_name if sender_name else ""}"""
    ...
    async def _inject_flashlite_context_impl(
        ...
        try:
            ...
            # 逐字原文召回纪律（D7 降档摘要不自知缺口修复）：
            # 真机实测——老话题被压成 summary/brief 注入后，主模型误以为是完整逐字记忆，
            # 被要求逐字复述时凭摘要 + 人设幻觉编造冒充原文（has_tool_calls=False 根本没查）。
            # 这里加一条硬性纪律：被要求逐字/精确原话/完整细节时必须先调 QQ_data_original
            # 召回真实原文。静态文案，拼在 inject_parts 静态区保持 KVCache 命中。
            inject_parts.append(
                "## 逐字原文召回纪律（最高优先级）\n"
                "你收到的历史对话上下文中 标注『压缩摘要』『分级记录』『summary/brief』的内容 "
                "都是压缩后的记录 不是逐字原文 你不掌握其确切措辞。\n"
                "当用户要求逐字复述/精确原话/完整细节（如『一字不差』『原话是什么』"
                "『把当时的话完整发出来』）时 必须先调用 QQ_data_original 工具按消息序号"
                "或关键词召回真实消息原文 再据召回结果回答。\n"
                "严禁凭记忆、摘要或人格设定编造、脑补或冒充原文——拿不到原文就如实说明 不要编。"
            )
