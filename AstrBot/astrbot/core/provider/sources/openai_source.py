...
class ProviderOpenAIOfficial(Provider):
    ...
    async def _query(self, payloads: dict, tools: ToolSet | None) -> LLMResponse:
        # [FlashLite-Debug] 追踪工具是否到达 API 层
        _dbg_model = payloads.get("model", "unknown")
        if tools:
            _dbg_names = tools.names() if hasattr(tools, 'names') else str(tools)
            logger.info(f"[FlashLite-Debug] _query 收到 {len(tools.tools) if hasattr(tools, 'tools') else '?'} 个工具, model={_dbg_model}, tools={_dbg_names[:200]}")
        else:
            logger.warning(f"[FlashLite-Debug] _query 收到 tools=None! model={_dbg_model}")
        if tools:
            ...
            if tool_list:
                ...
                logger.info(f"[FlashLite-Debug] 已注入 {len(tool_list)} 个工具到 API payload")
            else:
                logger.warning(f"[FlashLite-Debug] get_func_desc_openai_style 返回空列表!")
        ...
        # [FlashLite-Debug] 检查 API 原始响应中的 tool_calls
        _choice = completion.choices[0] if completion.choices else None
        _has_tc = bool(_choice and _choice.message.tool_calls) if _choice else False
        _tc_count = len(_choice.message.tool_calls) if _has_tc else 0
        _finish = _choice.finish_reason if _choice else "N/A"
        _content_len = len(_choice.message.content or "") if _choice else 0
        logger.info(
            f"[FlashLite-Debug] API 原始响应: finish_reason={_finish}, "
            f"has_tool_calls={_has_tc}, tool_calls_count={_tc_count}, "
            f"content_length={_content_len}"
        )
        if _has_tc:
            for _tc in _choice.message.tool_calls:
                logger.info(f"[FlashLite-Debug]   tool_call: {_tc.function.name}({_tc.function.arguments[:100]})")
    ...
    async def _parse_openai_completion(
        ...
        if choice.message.tool_calls and tools is not None:
            ...
            # [FlashLite-Debug] 打印所有可用工具名和 API 返回的 tool_call 名
            available_names = [t.name for t in tools.func_list]
            logger.info(f"[FlashLite-Debug] 可用工具名: {available_names}")
            for tool_call in choice.message.tool_calls:
                if isinstance(tool_call, str):
                    # workaround for #1359
                    tool_call = json.loads(tool_call)
                logger.info(f"[FlashLite-Debug] API 返回 tool_call: name='{tool_call.function.name}', 在可用工具中={'YES' if tool_call.function.name in available_names else 'NO!!!'}")
                if tools is None:
                    # 工具集未提供
                ...
                if not found:
                    logger.warning(f"[FlashLite-Debug] ⚠️ tool_call '{tool_call.function.name}' 未在 tools.func_list 中找到匹配!")
            logger.info(f"[FlashLite-Debug] 匹配结果: func_name_ls={func_name_ls}, 共{len(func_name_ls)}个")
