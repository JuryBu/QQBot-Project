...
class ProviderGoogleGenAI(Provider):
    ...
    def __init__(
        ...
        # KVCache 自动管理状态
        self._kv_cache_name: str | None = None
        self._kv_cache_hash: str | None = None
        self._kv_cache_enabled: bool = provider_config.get("kv_cache_enabled", True)

    async def _ensure_kv_cache(
        self,
        system_instruction: str | None,
        tools: ToolSet | None,
        model: str,
    ) -> str | None:
        """自动创建/复用 KVCache。返回 cache name 或 None（降级）。

        约束：使用 cached_content 时不能同时指定 system_instruction/tools。
        所以 system_instruction + tools 必须全部包含在缓存中。
        """
        if not self._kv_cache_enabled or not system_instruction:
            return None

        # 计算 hash（system_instruction + tool 完整签名）
        tool_sig = ""
        if tools and not tools.empty():
            try:
                ...
                if func_desc and "function_declarations" in func_desc:
                    # 包含函数名 + 参数 schema，防止参数变更时复用旧缓存
                    tool_sig = json.dumps(
                        func_desc["function_declarations"],
                        sort_keys=True,
                        ensure_ascii=False,
                    )
            except Exception:
        ...
        content_hash = hashlib.md5(
            f"{system_instruction}|{tool_sig}".encode("utf-8")
        ).hexdigest()

        # 如果 hash 一致，复用现有缓存
        if self._kv_cache_hash == content_hash and self._kv_cache_name:
        ...
        try:
            ...
            # 使用非 aio client 创建缓存（genai.Client 的同步版本）
            ...
            cache_config = types.CreateCachedContentConfig(
                ...
                display_name=f"astrbot_main_{content_hash[:8]}",
            )
            if tool_list:
                cache_config.tools = tool_list

            cache = sync_client.caches.create(config=cache_config)
            self._kv_cache_name = cache.name
            self._kv_cache_hash = content_hash
            logger.info(
                f"[KVCache] 创建缓存成功: {cache.name} "
                f"(hash={content_hash[:8]}, model={model})"
            )
            return cache.name

        except Exception as e:
            # 优雅降级：缓存创建失败不影响正常请求
            logger.warning(f"[KVCache] 缓存创建失败，降级为标准模式: {e}")
            self._kv_cache_enabled = False  # 本次会话不再尝试
            return None
    ...
    async def _prepare_query_config(
        ...
        logger.info(f"[FlashLite-Debug] _prepare_query_config: model={model_name}, tools={tools is not None and not tools.empty() if tools else False}, native_cr={native_coderunner}, native_s={native_search}, url_ctx={url_context}")
        ...
        if tools and tool_list:
            ...
            logger.info(f"[FlashLite-Debug] 函数工具已加载: {len(func_desc['function_declarations'])} 个 function_declarations")
        else:
            logger.info(f"[FlashLite-Debug] 没有加载任何工具: tools={tools is not None}, tool_list_before={tool_list}")
    ...
    async def _query(self, payloads: dict, tools: ToolSet | None) -> LLMResponse:
        ...
        # FlashLite KVCache 支持
        cached_content_name = payloads.get("_cached_content")
        # 自动缓存：外部未传时尝试自动创建
        if not cached_content_name and self._kv_cache_enabled:
            cached_content_name = await self._ensure_kv_cache(
                system_instruction, tools, model
            )
        ...
        while True:
            try:
                if cached_content_name:
                    # 使用缓存模式：system_instruction/tools/tool_config 已在缓存中
                    config = await self._prepare_query_config(
                        payloads,
                        None,  # tools 已在缓存中
                        "auto",
                        None,  # system_instruction 已在缓存中
                        modalities,
                        temperature,
                    )
                    config.cached_content = cached_content_name
                    logger.info(f"[KVCache] 使用缓存模式: {cached_content_name}")
                else:
                    config = await self._prepare_query_config(
                        payloads,
                        tools,
    ...
    async def _query_stream(
        ...
        while True:
            try:
                if cached_content_name:
                    config = await self._prepare_query_config(
                        payloads, None, "auto", None,
                    )
                    config.cached_content = cached_content_name
                    logger.info(f"[KVCache] 流式使用缓存模式: {cached_content_name}")
                else:
                    config = await self._prepare_query_config(
                        payloads,
                        tools,
