...
class RespondStage(Stage):
    ...
    async def initialize(self, ctx: PipelineContext) -> None:
        ...
        # 短句合并阈值
        self.merge_threshold = int(
            ctx.astrbot_config["platform_settings"]["segmented_reply"].get(
                "merge_threshold", 80
            )
        )
        # 自适应延迟档位（从配置读取）
        ad = ctx.astrbot_config["platform_settings"]["segmented_reply"].get(
            "adaptive_delays", {}
        )
        self.delay_short = self._parse_range(ad.get("short", "0.8,1.5"))
        self.delay_medium = self._parse_range(ad.get("medium", "1.5,3.0"))
        self.delay_long = self._parse_range(ad.get("long", "2.5,4.5"))
        # 最大分段数硬限
    ...
    async def _calc_comp_interval(self, comp: BaseMessageComponent) -> float:
        ...
        if self.interval_method == "adaptive":
            # 自适应模式：根据下一条消息的字数动态调整延迟
            if isinstance(comp, Comp.Plain):
                wc = await self._word_cnt(comp.text)
                if wc <= 15:
                    return random.uniform(*self.delay_short)
                elif wc <= 40:
                    return random.uniform(*self.delay_medium)
                else:
                    return random.uniform(*self.delay_long)
        ...
        if len(result.chain) > 0:
            ...
            if self.is_seg_reply_required(event):
                ...
                if not result.chain or len(result.chain) == 0:
                    ...
                    return
                # 短句合并：相邻 Plain comp 字数和 <= merge_threshold 时合并
                ...
                # 硬限分段数：超出 max_segments 的 Plain 强制合并到末段
                if self.max_segments > 0 and len(result.chain) > self.max_segments:
                    ...
                    keep.extend(non_plain)
                    result.chain = keep
                    logger.info(f"硬限分段: 原{len(merged_chain)}段 -> {len(result.chain)}段 (max_segments={self.max_segments})")

                # FlashLite emoji 插入：在文本分段完成后，将 emoji Image 插入到正确位置
                _emoji_path = event.get_extra("_emoji_path")
                if _emoji_path:
                    _send_after = event.get_extra("_emoji_send_after", 1)
                    _insert_idx = min(_send_after, len(result.chain))
                    result.chain.insert(_insert_idx, Comp.Image(file=_emoji_path))
                    logger.info(f"[Emoji] 插入到 chain[{_insert_idx}]: send_after={_send_after}, chain_len={len(result.chain)}")
                ...
                # 非分段模式下 emoji 追加到 chain 末尾（不存在"第几条之后"的概念）
                _emoji_path = event.get_extra("_emoji_path")
                if _emoji_path:
                    result.chain.append(Comp.Image(file=_emoji_path))
                    logger.info(f"[Emoji] 非分段模式，追加到 chain 末尾")
