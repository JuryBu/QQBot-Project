...
def _modalities_fix(provider: Provider, req: ProviderRequest) -> None:
    ...
    if req.func_tool:
        ...
        _tool_count = len(req.func_tool.tools) if hasattr(req.func_tool, 'tools') else '?'
        logger.info(f"[FlashLite-Debug] _modalities_fix: func_tool有{_tool_count}个工具, modalities={provider_cfg}")
        if "tool_use" not in provider_cfg:
            logger.warning(
                "[FlashLite-Debug] Provider %s modalities=%s 不含 tool_use, 清空工具!", provider, provider_cfg
            )
            req.func_tool = None
        else:
            logger.info(f"[FlashLite-Debug] _modalities_fix: ✅ tool_use 在 modalities 中，保留{_tool_count}个工具")
    else:
        logger.warning("[FlashLite-Debug] _modalities_fix: func_tool 为 None/空!（工具在 _modalities_fix 之前就不存在）")
    ...
    *,
    ...
    if req is None:
        if event.get_extra("provider_request"):
            ...
            logger.debug(f"[AGENT_BUILD_DEBUG] 开始遍历消息组件, 总数={len(event.message_obj.message)}, 类型={[type(c).__name__ for c in event.message_obj.message]}")
            for comp in event.message_obj.message:
                if isinstance(comp, Image):
                    ...
                    logger.info(f"[AGENT_BUILD_DEBUG] File组件: name={file_name}, path={file_path}, comp.name={comp.name}")
                    req.extra_user_content_parts.append(
                        TextPart(
                    ...
                    try:
                        video_url = comp.file or ""
                        req.extra_user_content_parts.append(
                            TextPart(text=f"[Video Attachment: {video_url}]")
                        )
                    except Exception as e:
                        logger.warning(f"Video attachment processing failed: {e}")
                elif isinstance(comp, Record):
                    try:
                        audio_url = comp.url or comp.file or ""
                        req.extra_user_content_parts.append(
                            TextPart(text=f"[Audio Attachment: {audio_url}]")
                        )
                    except Exception as e:
                        logger.warning(f"Record attachment processing failed: {e}")
                elif isinstance(comp, Share):
                    try:
                        share_url = comp.url or ""
                        share_title = comp.title or ""
                        share_content = comp.content or ""
                        req.extra_user_content_parts.append(
                            TextPart(
                                text=f"[Share Link: title={share_title}, url={share_url}, desc={share_content}]"
                            )
                        )
                    except Exception as e:
                        logger.warning(f"Share attachment processing failed: {e}")
                    ...
                    try:
                        ...
                        if isinstance(raw_data, str):
                            ...
                            card_data = {}
                        jump_url = ""
                        card_title = card_data.get("prompt", "")
                        ...
                        for key in meta:
                            ...
                            if isinstance(sub, dict):
                                ...
                                card_title = card_title or sub.get("title", "")
                        desc = f"title={card_title}" if card_title else ""
                        if jump_url:
                            desc += f", url={jump_url}"
                        req.extra_user_content_parts.append(
                            TextPart(text=f"[Card Message: {desc}]" if desc else "[Card Message]")
                        )
                        logger.info(f"[AGENT_BUILD_DEBUG] Json组件: title={card_title}, jump_url={jump_url}, raw_data_type={type(raw_data).__name__}")
                    except Exception as e:
                        logger.warning(f"Json card processing failed: {e}")
                elif isinstance(comp, Music):
                    try:
                        music_title = comp.title or ""
                        music_url = comp.url or ""
                        req.extra_user_content_parts.append(
                            TextPart(
                                text=f"[Music Share: title={music_title}, url={music_url}]"
                            )
                        )
                    except Exception as e:
                        logger.warning(f"Music attachment processing failed: {e}")
                elif isinstance(comp, Location):
                    try:
                        loc_title = comp.title or ""
                        lat = comp.lat
                        lon = comp.lon
                        map_url = f"https://uri.amap.com/marker?position={lon},{lat}&name={loc_title}"
                        req.extra_user_content_parts.append(
                            TextPart(
                                text=f"[Location: {loc_title}, map_url={map_url}]"
                            )
                        )
                    except Exception as e:
                    ...
                    try:
                        req.extra_user_content_parts.append(
                            TextPart(
                                text=f"[Forward Message: id={comp.id}, use media_summary tool to get content]"
                            )
                        )
                    except Exception as e:
                        logger.warning(f"Forward processing failed: {e}")
            # === 调试日志：组件循环结束后汇总 ===
            if req.extra_user_content_parts:
                parts_summary = [p.text[:80] for p in req.extra_user_content_parts if hasattr(p, 'text')]
                logger.info(f"[AGENT_BUILD_DEBUG] extra_user_content_parts汇总({len(req.extra_user_content_parts)}条): {parts_summary}")
    ...
    # [FlashLite硬化] TOOL_CALL_PROMPT 已移除
    # FlashLite 插件自带完整工具使用规范 (Section 7-16)
    # 原 TOOL_CALL_PROMPT 的 "briefly summarize result" 与简短输出约束冲突
