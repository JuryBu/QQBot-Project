...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    ...
    async def reset(
        ...
        **kwargs: T.Any,
        ...
        self._tool_content_yielded = False  # Gap1: 三态原子性——首次 tool step 可发文本，后续 suppress
    ...
    async def _iter_llm_responses(
        ...
        # FlashLite KVCache: 透传缓存名（如果插件设置了动态属性）
        _cc = getattr(self.req, "_cached_content", None)
        if _cc:
            payload["_cached_content"] = _cc
        if include_model:
    ...
    async def step(self):
        ...
        # 返回 LLM 结果——三态原子性设计
        # 1. 纯消息（无工具调用）→ 正常 yield
        # 2. 消息+工具（首次 tool step）→ yield 文本 + 执行工具（用户看到一次消息）
        # 3. 纯工具 / 后续 tool step → suppress 文本，避免重复
        # 文本始终会被记录到上下文（L619+），模型知道自己"说过"什么。
        if not llm_resp.tools_call_name:
            # 情况 1: 无工具调用——最终回复，正常发送
        ...
        elif not self._tool_content_yielded:
            # 情况 2: 首次 tool step 且有文本——"消息+工具"模式，发送一次
            if llm_resp.result_chain:
                yield AgentResponse(
                    type="llm_result",
                    data=AgentResponseData(chain=llm_resp.result_chain),
                )
                self._tool_content_yielded = True
            elif llm_resp.completion_text:
                yield AgentResponse(
                    type="llm_result",
                    data=AgentResponseData(
            ...
            # 情况 3: 后续 tool step——suppress 文本，防止重复
            logger.debug(
                f"Suppressing repeated tool-step text (already sent once): "
                f"{llm_resp.completion_text[:80]}..."
            )
    ...
    async def step_until_done(
        ...
        while not self.done() and step_count < max_step:
            step_count += 1
            # Gap5: 接近上限时注入预警
            await self._inject_step_warning_if_needed(step_count, max_step)
    ...
    async def _inject_step_warning_if_needed(
        self, step_count: int, max_step: int
    ) -> None:
        """Gap5: 在接近 max_step 时注入警告，让模型自行优雅收尾。"""
        remaining = max_step - step_count
        if remaining == 3:
            self.run_context.messages.append(
                Message(
                    role="user",
                    content=(
                        f"[系统提示] 工具调用还剩 {remaining} 步即将达到上限。"
                        "请尽快完成当前操作并准备总结回复。如果还有必要的工具调用请立即执行，否则直接回复用户。"
                    ),
                    _no_save=True,
                )
            )
            logger.info(f"Injected step warning: {remaining} steps remaining")
